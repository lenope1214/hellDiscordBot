...
import kr.weareboard.runner.SHOW_BUTTONS
...
fun TextChannel.deleteAllMessages() {
    try {
        ...
        iterableMessage
            .takeAsync(1) // Collect 5 messages
            .thenApply { it ->

                it.forEach { m ->
                    log.info("삭제할 메세지 : ${m.contentRaw}, ${m.embeds.joinToString { it.title ?: "" }}")
                    // 삭제요청한 시간보다 5초 이전에 만들어진 데이터만 삭제
                    if (m.timeCreated.isBefore(deleteTime)) {
                        // 공지가 포함되면 삭제하지 않는다.
                        if (m.contentRaw.contains("[공지]").not()) {
                            m.delete().queue()
                        }
                    }
                }
    ...
    this.deleteAllMessages().let {
        ...
        sendMessageEmbeds
            .queueAfter(1, TimeUnit.SECONDS) {
                log.info("새로운 메세지 생성 완료 message id : ${it.idLong}")
            }
    }
}
